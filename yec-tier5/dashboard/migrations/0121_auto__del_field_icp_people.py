...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Deleting field 'ICP.people'
        db.delete_column(u'dashboard_icp', 'people_id')

        # Adding M2M table for field people on 'ICP'
        m2m_table_name = db.shorten_name(u'dashboard_icp_people')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('icp', models.ForeignKey(orm[u'dashboard.icp'], null=False)),
            ('person', models.ForeignKey(orm[u'legion.person'], null=False))
        ))
        db.create_unique(m2m_table_name, ['icp_id', 'person_id'])


    def backwards(self, orm):
        # Adding field 'ICP.people'
        db.add_column(u'dashboard_icp', 'people',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['legion.Person'], null=True, blank=True),
                      keep_default=False)

        # Removing M2M table for field people on 'ICP'
        db.delete_table(db.shorten_name(u'dashboard_icp_people'))
