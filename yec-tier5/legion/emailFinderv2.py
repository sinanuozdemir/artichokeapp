...
class Emailgorithm():
	def __init__(self, legion_object, stop_if_matches, want = ['personal', 'company']):
		self.legion_object = legion_object
		self.scrapers = legion_object.scrapers
		self.person = legion_object.information
		self.stop_if_matches = stop_if_matches
		self.want = want
	...
	def match(self, email):
		if not self.isValidEmail(email):
			return False
		potential_person = c_cubed.Scanner(email_to_find = email, fuzzy = False, stop_if_matches = self.stop_if_matches).ScanForSocialMedias()
		print "checking if the same person"
	...
	def _getPermutationsOfEmail(self, first, last, company_host): #assume the per determiend patter has the @ symbol
		try:
			return [f+company_host for f in [first, first+'.'+last, first+last, first[0]+last]]
		except:
			return []
		return []
